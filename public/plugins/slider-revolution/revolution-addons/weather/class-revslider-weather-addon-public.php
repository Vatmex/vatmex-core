...
class Revslider_Weather_Addon_Public
{
    ...
    public function revslider_add_layer_html($slider, $slide)
    {
        ...
        if (isset($sliderParams['revslider-weather-enabled']) && $sliderParams['revslider-weather-enabled'] == 'true') {
            wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__).'css/revslider-weather-addon-public.css', [], $this->version, 'all');
        }
        ...
        if (isset($sliderParams['revslider-weather-enabled']) && $sliderParams['revslider-weather-enabled'] == 'true' && ! empty($sliderParams['revslider-weather-refresh'])) {
            wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__).'js/revslider-weather-addon-public.js', ['jquery'], $this->version, false);
            wp_localize_script($this->plugin_name, 'rev_slider_weather_addon', [
    ...
    public function rev_addon_insert_meta($record)
    {
        ...
        if (strpos($record['layers'], '%weather_icon%')) {
            wp_enqueue_style($this->plugin_name.'_icons', plugin_dir_url(__FILE__).'css/revslider-weather-addon-icon.css', [], $this->version, 'all');
        }
            ...
            foreach ($matches as $forecast_array) {
                if (is_array($forecast_array)) {
                    foreach ($forecast_array as $forecast) {
                        ...
                        $forecast = str_replace(['%', '_forecast', 'weather_'], '', $forecast);
